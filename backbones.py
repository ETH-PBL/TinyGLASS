...
def _patch_torchvision_resnet_no_inplace_add(model):
    """Avoid in-place residual adds in torchvision ResNet blocks.

    SDSP tiling advisor can fail when an Add is emitted in-place (top==bottom).
    Rewriting `out += identity` to `out = out + identity` keeps semantics but
    avoids in-place graph artifacts.
    """
    try:
    ...
    # Ensure relus are not in-place (safer for exporters).
...
def load(name):
    m = eval(_BACKBONES[name])
    return _patch_torchvision_resnet_no_inplace_add(m)
