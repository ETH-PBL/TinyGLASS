...
class MVTecDataset(torch.utils.data.Dataset):
    ...
    def get_image_data(self):
        ...
        if self.contamination_rate > 0.0 and self.split == DatasetSplit.TRAIN:
            test_path = os.path.join(self.source, self.classname, "test")
            anomaly_types = [a for a in os.listdir(test_path) if a != "good"]
            all_anomaly_paths = []
            for anomaly in sorted(anomaly_types):
                ap = os.path.join(test_path, anomaly)
                all_anomaly_paths.extend([os.path.join(ap, f) for f in sorted(os.listdir(ap))])

            n_good = len(data_to_iterate)
            n_contaminated = min(int(n_good * self.contamination_rate), len(all_anomaly_paths))
            rng = np.random.RandomState(42)
            chosen = rng.choice(len(all_anomaly_paths), n_contaminated, replace=False)
            for idx in chosen:
                data_to_iterate.append([self.classname, "good", all_anomaly_paths[idx], None])
